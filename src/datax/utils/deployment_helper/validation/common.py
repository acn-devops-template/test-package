...
def check_path_endswith_dot_json(v: str) -> str:
    ...
    if file_extension != '.json':
        raise ValueError(f"Incorrect file format for the path `{v}`, the file should be `.json`")
    return v
...
class CommandlineArgumentValidator(BaseModel):
    ...
    _check_date_format = validator('start_date', 'end_date', allow_reuse=True)(check_date_format)
    _check_start_date_is_before_end_date = root_validator(allow_reuse=True)(check_start_date_is_before_end_date)
...
class PipelineConfigArgumentValidators(BaseModel, extra=Extra.allow):
    ...
    _check_path_endswith_dot_json = validator('output_schema_path', allow_reuse=True)(check_path_endswith_dot_json)
...
class TransformationConfigArgumentValidator(BaseModel, extra=Extra.allow):
    ...
    _check_path_endswith_dot_json = validator('input_schema_path', 'ref_schema_path', allow_reuse=True)(check_path_endswith_dot_json)
